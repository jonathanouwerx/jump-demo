...
export async function executeCommand(command, fileSystem) {
    const [cmd, ...args] = command.trim().split(/\s+/);
    if (!cmd)
        return "";
    // Find matching command or alias
    const matchedCommand = Object.entries(commands).find(([name, { aliases }]) => name === cmd.toLowerCase() ||
        (aliases && aliases.includes(cmd.toLowerCase()))) || null;
    if (matchedCommand) {
